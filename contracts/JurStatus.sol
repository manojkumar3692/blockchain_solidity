...
contract JurStatus {
    ...
    */
    ...
    /** Dynamic array holding jur status */
    string[] public statusList;
    ...
    */
    ...
    onlyOwner {
        ...
        statusCount++;
        addedStatusList = statusTypes[_statusType];
        statusList.push(addedStatusList);
        emit StatusAdded(_statusHolder, now, statusTypes[_statusType]);
    }

    function getJurStatus() public view returns(string[] memory) {
        return statusList;
    }
    ...
    */
    ...
    function getStatusType() public view returns(string[] memory) {
        return statusTypes;
    }

    // function getJurStatus(address _address) public view returns(uint memory activationTime,bool memory isActive,string memory statusType) {
    //      //if(users[_address].age == 0) throw;
    //     return (status[_address].activationTime, status[_address].isActive, status[_address].statusType);        
    // }
